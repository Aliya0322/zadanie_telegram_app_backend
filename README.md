...
- `GET /api/v1/groups/{group_id}` - Получить группу по ID (доступно для учителя группы или её участников)
  - Path-параметр: `group_id` (integer) - ID группы
  - Требует авторизации через заголовок `X-Telegram-Init-Data`
  - Возвращает 404, если группа не найдена
  - Возвращает 403, если пользователь не является учителем или участником группы
- `GET /api/v1/groups/{group_id}/invite-link` - Получить ссылку-приглашение для группы (только для учителя группы)
