...
- `POST /api/v1/homework/` - Создать домашнее задание для группы (только для учителей)
  - Принимает: `group_id`, `description`, `deadline`
  - Устанавливает домашнее задание для группы по ID
  - Триггерирует планировщик для отправки уведомлений за 1 час до дедлайна
  - Требует авторизации через заголовок `X-Telegram-Init-Data`
  - Возвращает 404, если группа не найдена
  - Возвращает 403, если пользователь не является учителем группы
- `PUT /api/v1/homework/{homework_id}` - Редактировать домашнее задание (только для учителей)
  - Path-параметр: `homework_id` (integer) - ID домашнего задания
  - Принимает: `description`, `deadline` (все поля опциональные)
  - При изменении дедлайна автоматически перепланируется напоминание
  - Требует авторизации через заголовок `X-Telegram-Init-Data`
  - Возвращает 404, если задание не найдено
  - Возвращает 403, если пользователь не является учителем группы
- `DELETE /api/v1/homework/{homework_id}` - Удалить домашнее задание (только для учителей)
  - Path-параметр: `homework_id` (integer) - ID домашнего задания
  - Автоматически отменяет запланированное напоминание
  - Требует авторизации через заголовок `X-Telegram-Init-Data`
...
- `POST /api/v1/schedule/` - Создать занятие в расписании (только для учителей)
  - Принимает: `group_id`, `day_of_week`, `time_at`, `meeting_link` (опционально)
  - Устанавливает расписание для группы по ID
  - Требует авторизации через заголовок `X-Telegram-Init-Data`
  - Возвращает 404, если группа не найдена
  - Возвращает 403, если пользователь не является учителем группы
- `PUT /api/v1/schedule/{schedule_id}` - Редактировать занятие в расписании (только для учителей)
  - Path-параметр: `schedule_id` (integer) - ID занятия в расписании
  - Принимает: `day_of_week`, `time_at`, `meeting_link` (все поля опциональные)
  - Требует авторизации через заголовок `X-Telegram-Init-Data`
  - Возвращает 404, если занятие не найдено
  - Возвращает 403, если пользователь не является учителем группы
- `DELETE /api/v1/schedule/{schedule_id}` - Удалить занятие из расписания (только для учителей)
  - Path-параметр: `schedule_id` (integer) - ID занятия в расписании
  - Требует авторизации через заголовок `X-Telegram-Init-Data`
  - Возвращает 204 при успешном удалении
  - Возвращает 404, если занятие не найдено
